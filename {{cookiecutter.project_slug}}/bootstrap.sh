...
git clone --single-branch --depth 1 https://github.com/{{ cookiecutter.github_username }}/{{ cookiecutter.project_slug }}.git
pushd {{ cookiecutter.project_slug }}
make dist
pip install dist/{{ cookiecutter.project_slug }}*whl
# copy the http configuration file
cp {{ cookiecutter.service_name.lower() }}.httpconf /etc/httpd/conf.d/{{ cookiecutter.service_name.lower() }}_rest.conf
popd

mkdir /var/www/{{ cookiecutter.service_name.lower() }}_rest

# write the WSGI file
cat <<EOF > /var/www/ddot_rest/{{ cookiecutter.project_slug }}.wsgi
#!/usr/bin/env python

import os
os.environ['{{ cookiecutter.service_name.upper() }}_REST_SETTINGS']="/var/www/{{ cookiecutter.project_slug }}/{{ cookiecutter.service_name.lower() }}_rest.cfg"

from {{ cookiecutter.project_slug }} import app as application
EOF

# write the configuration file
cat <<EOF > /var/www/{{ cookiecutter.project_slug }}/{{ cookiecutter.service_name.lower() }}_rest.cfg
WAIT_COUNT=600
SLEEP_TIME=1
EOF
...
semanage fcontext -a -t httpd_sys_rw_content_t "/var/www/{{ cookiecutter.service_name.lower() }}_rest/tasks(/.*)?"
restorecon -Rv /var/www/{{ cookiecutter.service_name.lower() }}_rest/tasks

service httpd start

echo "Visit http://localhost:8081/{{ cookiecutter.service_name.lower() }}/rest/v1 in your browser"
