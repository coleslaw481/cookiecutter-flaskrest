...
{{ cookiecutter.service_name.upper() }}_REST_SETTINGS_ENV = '{{ cookiecutter.service_name.upper() }}_REST_SETTINGS'
# global api object
app = Flask(__name__)
...
app.config.from_envvar({{ cookiecutter.service_name.upper() }}_REST_SETTINGS_ENV, silent=True)
app.logger.info('Job Path dir: ' + app.config[JOB_PATH_KEY])
SERVICE_NS = '{{ cookiecutter.service_name.lower() }}'
...
class Run{{ cookiecutter.service_name }}(Resource):
    """
    Runs {{ cookiecutter.service_name }}
    """
    ...
    @api.doc('Runs {{ cookiecutter.service_name }}')
    ...
    def post(self):
        ...
        try:
            params = Run{{ cookiecutter.service_name }}.post_parser.parse_args(request, strict=True)
